...
- For `entityType` of `fire`, generally the more flames the greater the damage, burn time (internal Factorio logic) and will make a slightly wider fire area. Values at the higher end seem to have a diminishing effect in-game. Suggested is 50, with the max of 250. It takes 20 fire count to set a tree on fire, but at this level the player will have to run right next to a tree to set it on fire. A suggested value is 50, which generally sets trees very close to the player on fire without requiring the player to actually touch them. Max is 250, but greater numbers seeming to have a diminishing effect in-game. Value capped at 250, as the games maximum of 255 can behave oddly, see bug report: https://forums.factorio.com/viewtopic.php?f=7&t=103227
