...
- For `customEntityName` of type `fluid-turret`: use the `customSecondaryDetail` to specify the fluid type to load the turret with. When using the `ammoCount` setting be aware that the amount must be greater than the turrets minimum fluid to activate, for vanilla Flamethrower turrets this is greater than 25.
